...
) const
{
    ...
    {
        case 0:
            return max(Lt()/(CDES*this->delta()), scalar(1));
        case 1:
            return max(Lt()*(1 - F1)/(CDES*this->delta()), scalar(1));
        case 2:
            return max(Lt()*(1 - F2)/(CDES*this->delta()), scalar(1));
        default:
...
) const
{
    ...
    //volScalarField CDES(this->CDES(F1));
    return this->betaStar_*this->omega_()*FDES(F1, F23)()();
...
:
    ...
    FSST_
    (
        this->coeffDict_.template getOrDefault<label>("FSST", 2)
    )
{
    correctNut();
...
tmp<volScalarField> kOmegaSSTDES<BasicTurbulenceModel>::LESRegion() const
{
    ...
    const volScalarField& F23(this->F2());
