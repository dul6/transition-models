...
tmp<volScalarField> kOmegaSSTLMDES<BasicTurbulenceModel>::F1
(
...
{
    return gammaIntEff_*kOmegaSSTDES<BasicTurbulenceModel>::Pk(G);
}
...
) const
{
    return
       ...
       *kOmegaSSTDES<BasicTurbulenceModel>::epsilonByk(F1, gradU);//no problem
}
...
template<class BasicTurbulenceModel>
void kOmegaSSTLMDES<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
