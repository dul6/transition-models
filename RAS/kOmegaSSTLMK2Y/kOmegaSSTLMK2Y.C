...
tmp<volScalarField> kOmegaSSTLMK2Y<BasicTurbulenceModel>::F1
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLMK2Y<BasicTurbulenceModel>::Pk
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLMK2Y<BasicTurbulenceModel>::epsilonByk
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLMK2Y<BasicTurbulenceModel>::Fthetat
(
...
kOmegaSSTLMK2Y<BasicTurbulenceModel>::ReThetac() const
{
...
*/
        ReThetac[celli] = 967.34*pow(Tuinf_, -1.0315);
    }
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLMK2Y<BasicTurbulenceModel>::Flength
(
    const volScalarField::Internal& nu
) const
{
...
*/
            Flength[celli] = 10.435*pow(Tuinf_, 2.9756);
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLMK2Y<BasicTurbulenceModel>::ReThetat0
(
    const volScalarField::Internal& Us,
...
)
{
    ...
    forAll(ReThetat0, celli)
    {
        ...
        do
        {
            ...
            if (Tu <= 1.3)
            {
                const scalar Flambda =
                  ...
                  - (
                     - 12.986*lambdai
                     - 123.66*sqr(lambdai)
                     - 405.689*pow3(lambdai)
                    )*exp(-pow(Tu/1.5, 1.5))
                  :
                    1
                  + 0.275*(1 - exp(-35*lambdai))
                   *exp(-Tu/0.5);
            ...
            lambdai = sqr(thetat)/nu[celli]*dUsds[celli];
            lambdai = max(min(lambdai, 0.1), -0.1);

            lambdaErr = mag(lambdai - lambda0);

            maxIter = max(maxIter, ++iter);

        } while (lambdaErr > lambdaErr_);

        lambda_[celli] = lambdai;
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLMK2Y<BasicTurbulenceModel>::Fonset
(
...
template<class BasicTurbulenceModel>
kOmegaSSTLMK2Y<BasicTurbulenceModel>::kOmegaSSTLMK2Y
(
...
:
    ...
    Tuinf_
    (
        this->coeffDict_.lookupOrDefault("Tuinf", 0.9)
    ),
    ...
    (
        IOobject
        (
            IOobject::groupName("lambda", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->mesh_
        ),
        this->mesh_,
...
template<class BasicTurbulenceModel>
bool kOmegaSSTLMK2Y<BasicTurbulenceModel>::read()
{
    if (kOmegaSST<BasicTurbulenceModel>::read())
    {
        ...
        this->coeffDict().readIfPresent("Tuinf", Tuinf_);
...
template<class BasicTurbulenceModel>
void kOmegaSSTLMK2Y<BasicTurbulenceModel>::correctReThetatGammaInt()
{
    // Local references
    ...
    (
        min(20*(0.1 - mag(lambda_))*max(Rev/(3.235*ReThetac) - 1, scalar(0))*Freattach, scalar(2))
       *Fthetat
    );
...
template<class BasicTurbulenceModel>
void kOmegaSSTLMK2Y<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
